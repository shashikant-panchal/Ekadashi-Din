...
const getChapterVerses = (chapterId, verseCount) => {
  ...
  return Array.from({ length: Math.min(verseCount, 5) }, (_, i) => ({
    ...
    translation: `This is verse ${i + 1} of Chapter ${chapterId}: ${
      chapter?.englishName || "Bhagavad Gita"
...
const ChapterListItem = ({
  ...
  <View>
    ...
    >
      ...
      <TouchableOpacity
        style={[
          chapterStyles.continueButton,
          { backgroundColor: colors.card, borderColor: colors.border },
        ]}
        onPress={onPress}
      >
        <ThemedText
          style={[chapterStyles.buttonText, { color: colors.foreground }]}
        >
          Continue
        </ThemedText>
      </TouchableOpacity>
...
const DailyReading = ({ navigation }) => {
  ...
  const loadReadingData = async () => {
    ...
    try {
      ...
      const calculatedStats = ReadingService.calculateReadingStats(
        progressData,
        chapters
      );
  ...
  const handleMarkAsRead = async () => {
    ...
    try {
      const result = await ReadingService.markVerseComplete(
        user.id,
        verse.chapter,
        verse.verse
      );
  ...
  const handleOpenChapter = async (chapter) => {
    ...
    try {
      ...
      const fallbackVerses = getChapterVerses(
        chapterId,
        chapter.verseCount || chapter.totalVerses
      );
        ...
        >
          ...
          >
            ...
            <View style={mStyles.headerCenter}>
              <ThemedText
                type="subtitle"
                style={[mStyles.chapterTitle, { color: colors.foreground }]}
              >
                Chapter {selectedChapter?.id || selectedChapter?.chapter}
              </ThemedText>
              {/* <ThemedText
                style={[
                  mStyles.chapterSubtitle,
                  { color: colors.mutedForeground },
                ]}
              >
                {chapterEnglishName}
            ...
            <ThemedText
              style={[mStyles.verseCounter, { color: colors.mutedForeground }]}
            >
              {currentVerseIndex + 1} / {totalVerseCount}
            </ThemedText>
          </View>
          ...
          >
            {modalLoading ? (
              ...
              <>
                ...
                <View
                  style={[
                    mStyles.sanskritBanner,
                    { backgroundColor: colors.primary },
                  ]}
                >
                  <ThemedText
                    type="devanagari"
                    style={mStyles.sanskritBannerText}
                  >
                    {currentModalVerse.sanskrit ||
                      currentModalVerse.text ||
                      currentModalVerse.slok ||
                      "Loading..."}
                  </ThemedText>
                </View>
                ...
                {/* Verse Card */}
                <View
                  style={[mStyles.verseCard, { backgroundColor: colors.card }]}
                >
                    ...
                    <View style={mStyles.verseLabelContainer}>
                      ...
                      >
                        ...
                        {currentModalVerse.verse ||
                          currentModalVerse.verse_number ||
                          currentVerseIndex + 1}
                      </ThemedText>
                    </View>
                    <TouchableOpacity
                  ...
                  >
                    ...
                    >
                      {currentModalVerse.sanskrit ||
                        currentModalVerse.text ||
                        currentModalVerse.slok ||
                        "Sanskrit text loading..."}
                    </ThemedText>
                    {currentModalVerse.transliteration ? (
                      <ThemedText
                        style={[
                          mStyles.verseTransliteration,
                          { color: colors.mutedForeground },
                        ]}
                      >
                        {currentModalVerse.transliteration}
                  ...
                  <View style={mStyles.translationSection}>
                    ...
                    <ThemedText
                      style={[
                        mStyles.translationText,
                        { color: colors.mutedForeground },
                      ]}
                    >
                      {currentModalVerse.translation ||
                        currentModalVerse.meaning ||
                        "Translation loading..."}
                    </ThemedText>
                  </View>
                  ...
                  >
                    ...
                    <ThemedText
                      style={[
                        mStyles.significanceText,
                        { color: colors.mutedForeground },
                      ]}
                    >
                      {currentModalVerse.significance ||
                        currentModalVerse.commentary ||
                        currentModalVerse.purport ||
                        "Significance will be available soon"}
                    </ThemedText>
                  </View>
                </View>
              </>
          ...
          >
            <TouchableOpacity
              style={[
                ...
                currentVerseIndex === 0 && mStyles.disabledButton,
              ]}
            ...
            >
              <ThemedText type="defaultSemiBold" style={mStyles.nextButtonText}>
                Next
              </ThemedText>
            ...
            <TouchableOpacity
              style={mStyles.markReadButton}
              onPress={handleModalMarkAsRead}
            >
              <AntDesign name="check" size={16} color={colors.primary} />
              <ThemedText
                style={[mStyles.markReadText, { color: colors.primary }]}
              >
                Read
              </ThemedText>
            </TouchableOpacity>
