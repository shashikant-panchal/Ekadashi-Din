...
const DateCell = ({ date, isEkadashi, isToday, isFestival, isMoon, colors, isDark }) => {
  ...
  if (isToday) {
    ...
    if (isEkadashi) {
      containerStyle.push({ backgroundColor: isDark ? '#3A1D1D' : '#FFF5F5' }); // Darker pink for dark mode
      dot = <View style={[styles.dot, { backgroundColor: '#EF4444' }]} />;
    } else if (isFestival) {
      containerStyle.push({ backgroundColor: isDark ? '#3A2E0E' : '#FEFCE8' }); // Darker yellow for dark mode
