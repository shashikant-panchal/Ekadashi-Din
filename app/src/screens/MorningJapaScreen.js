      ...
      <ScrollView showsVerticalScrollIndicator={false}
        ...
        <View style={styles.headerTitleContainer}>
          <View
            style={[
              styles.headerIconBg,
              { backgroundColor: isDark ? colors.muted : "#FFFBE6" },
            ]}
          >
        ...
        <View style={[styles.roundCard, { backgroundColor: colors.card }]}>
          <ThemedText
            type="subtitle"
            style={[styles.roundTitle, { color: colors.foreground }]}
          >
            Round {roundsCompleted + 1}
          </ThemedText>
          <ThemedText
            style={[styles.roundTime, { color: colors.mutedForeground }]}
          >
            {formatTime(time)}
          </ThemedText>
          <TouchableOpacity
          ...
          >
            ...
            <ThemedText
              ...
              style={[styles.startButtonText, { color: colors.foreground }]}
            >
              {isRunning ? "Pause Japa" : "Start Japa"}
            </ThemedText>
          </TouchableOpacity>
        </View>
        ...
        >
          {(() => {
            const hareKrishnaMantra =
              mantrasList.find((m) => m.name === "Hare Krishna Mahamantra") ||
              mantrasList[0];
            return (
              <>
                <ThemedText
                  type="defaultSemiBold"
                  style={styles.mantraCardTitle}
                >
                  {hareKrishnaMantra.name}
                </ThemedText>
                {hareKrishnaMantra.sanskrit.split("\n").map((line, index) => (
                  <ThemedText
                    type="devanagari"
                    key={index}
                    style={styles.sanskritMantra}
                  >
                    {line}
                  </ThemedText>
                ))}
                {hareKrishnaMantra.transliteration
                  .split("\n")
                  .map((line, index) => (
                    <ThemedText key={index} style={styles.englishMantra}>
                      {line}
                    </ThemedText>
                  ))}
              </>
        ...
        >
          ...
          >
            Complete Round {roundsCompleted + 1}
          </ThemedText>
        </TouchableOpacity>
        ...
        <View style={[styles.japaTipsCard, { backgroundColor: colors.card }]}>
          <View style={styles.japaTipsHeader}>
            ...
            <ThemedText
              type="subtitle"
              style={[styles.japaTipsTitle, { color: colors.foreground }]}
            >
              Japa Tips
            </ThemedText>
          </View>
          <View style={styles.tipItem}>
            <ThemedText style={[styles.bullet, { color: colors.foreground }]}>
              •
            </ThemedText>
            <ThemedText style={[styles.tipText, { color: colors.foreground }]}>
              Chant clearly and attentively
            </ThemedText>
          </View>
          <View style={styles.tipItem}>
            <ThemedText style={[styles.bullet, { color: colors.foreground }]}>
            ...
            <ThemedText style={[styles.tipText, { color: colors.foreground }]}>
              Complete 16 rounds daily before breakfast
            </ThemedText>
          </View>
          <View style={styles.tipItem}>
            <ThemedText style={[styles.bullet, { color: colors.foreground }]}>
            ...
            <ThemedText style={[styles.tipText, { color: colors.foreground }]}>
              Focus on hearing the holy names
            </ThemedText>
          </View>
          <View style={styles.tipItem}>
            <ThemedText style={[styles.bullet, { color: colors.foreground }]}>
            ...
            <ThemedText style={[styles.tipText, { color: colors.foreground }]}>
              Average time per round: 7-8 minutes
            </ThemedText>
          </View>
        </View>
      </ScrollView>
